 ...
 * Note that an AbstractGate could be made from many, smaller AbstractGates
 * 
 * @author Greg
 * 
 */
public abstract class AbstractGate {
     ...
     * 
     * @param numInputs
     *            The number of input jacks the gate will have
     * @param numOutputs
     *            The number of output jacks the gate will have
     */
	...
	if (0 <= port && port < inputJacks.length) {
	    ...
	    throw new IllegalArgumentException(
		    "AbstractGate.setInput(i) i out of bounds!");
    }
	...
	if (this.inputJacks.length == inputs.length) {
	    ...
	    throw new IllegalArgumentException(
		    "AbstractGate.setInputs() incompatible input array!");
    }
	...
	if (0 <= port && port < outputJacks.length)
	    ...
	    throw new IllegalArgumentException(
		    "AbstractGate.setOutput(i) i out of bounds!");
    }

    /**
     * Evaluate the input jacks, and update the output jacks according to the
     * logic rules of the gate
     */
